...
const StartOverlay: React.FC = () => {
	...
	// Persist in sessionStorage so retries/new passages do not show the overlay again.
	...
	// If the user starts typing (testStatus goes to 'running'), mark the overlay as seen.
	useEffect(() => {
	...
	// Overlay is visible only when it's the first visit AND the test status is idle/ready.
	const isVisible =
		isFirstVisit && (game.testStatus === "ready" || game.testStatus === "idle");
	...
	const handleStartClick = () => {
		// set flag so we don't show overlay again this session
