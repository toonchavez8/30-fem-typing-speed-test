...
import { Difficulty, Passage, PassageBuckets } from './types';
...
const buildDataset = (): PassageBuckets => {
  ...
  return (Object.entries(rawDataset) as [Difficulty, RawPassage[]][]).reduce<PassageBuckets>((acc, [difficulty, passages]) => {
...
export const getRandomPassage = (difficulty: Difficulty, excludeId?: string): Passage => {
  const bucket = dataset[difficulty];
  ...
  const pool = excludeId ? bucket.filter((passage) => passage.id !== excludeId) : bucket;
...
export const getNextPassageId = (currentId: string, difficulty: Difficulty): string => {
  const bucket = dataset[difficulty];
  ...
  const nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % bucket.length;
