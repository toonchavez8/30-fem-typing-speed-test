...
import { TypingMetrics, TypingResult, TypingTestStatus } from '../types';
...
type TypingTestAction =
  ...
  | { type: 'UPDATE_TYPED'; value: string; status: TypingTestStatus; isComplete: boolean };
...
const reducer = (state: TypingTestState, action: TypingTestAction): TypingTestState => {
  switch (action.type) {
    case 'RESET':
      return {
        ...initialState,
    ...
    case 'UPDATE_TYPED':
      return {
        ...
        startedAt: state.startedAt ?? (action.value.length > 0 ? Date.now() : null),
...
export const useTypingTest = (options: UseTypingTestOptions): UseTypingTestReturn => {
  ...
  const handleInput = useCallback(
    (nextValue: string) => {
      ...
      const isComplete = limitedValue.length === passageLength && passageLength > 0;
      const nextStatus: TypingTestStatus = limitedValue.length === 0 ? readyStatus : isComplete ? 'completed' : 'running';
  ...
  const normalizedTyped = useMemo(() => normalizeValue(state.typedValue), [state.typedValue]);
  ...
  const metrics = useMemo<TypingMetrics>(() => {
    ...
    const remainingCharacters = Math.max(targetChars.length - typedChars.length, 0);
    const progress = targetChars.length === 0 ? 0 : typedChars.length / targetChars.length;
    const accuracy = typedChars.length === 0 ? 1 : correctCharCount / typedChars.length;
