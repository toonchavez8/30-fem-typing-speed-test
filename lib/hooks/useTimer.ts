...
import { TimerSnapshot, TimerStatus } from '../types';
...
export const useTimer = (options: UseTimerOptions = {}): UseTimerReturn => {
  const { duration = null, autoStart = false, tickRate = DEFAULT_TICK_RATE, onTick, onComplete } = options;

  const durationMs = typeof duration === 'number' && duration > 0 ? duration * 1000 : null;
  ...
  const getElapsed = useCallback(() => {
    ...
    return pausedAccumulatedRef.current + (Date.now() - startTimestampRef.current);
  }, []);

  const handleCompletion = useCallback((finalElapsed?: number) => {
    clearTimer();
    startTimestampRef.current = null;
    const resolvedElapsed = typeof finalElapsed === 'number' ? finalElapsed : durationMs ?? getElapsed();
  ...
  const snapshot: TimerSnapshot = useMemo(() => {
    return {
      ...
      progress: durationMs === null ? null : Math.min(elapsedMs / durationMs, 1)
    };
